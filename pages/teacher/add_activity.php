...
$inventory = $db->getInventoryShop(); // Pre-load all inventory items
$categories = $db->getCategories(); // Pre-load categories for filtering
$class_id_from_url = $_GET['class_id'] ?? null;

$edit_mode = false;
$activity_data_for_js = 'null';

if (isset($_GET['edit_id'])) {
    $edit_mode = true;
    $activity_id = $_GET['edit_id'];
    
    $activity_details = $db->getActivityDetails($activity_id);
    if ($activity_details) {
        $activity_requirements = $db->getActivityRequirements($activity_id);
        $activity_assignments = $db->getAssignedClassesForActivity($activity_id);

        $js_data = [
            'details' => $activity_details,
            'requirements' => $activity_requirements,
            'assignments' => array_column($activity_assignments, 'ClassID')
        ];
        $activity_data_for_js = json_encode($js_data);
    } else {
        $edit_mode = false; 
    }
}
...
<body class="bg-[#f8fafc] min-h-screen font-sans text-slate-600">
    ...
    <div class="flex min-h-screen">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <main class="p-8" x-data="activityWizard(<?= htmlspecialchars(json_encode($inventory)) ?>, <?= htmlspecialchars(json_encode($categories)) ?>, <?= htmlspecialchars($activity_data_for_js, ENT_QUOTES, 'UTF-8') ?>)">
                <header class="mb-10">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
                    ...
                    <section class="lg:col-span-2 bg-white p-8 rounded-3xl border border-slate-200/50 shadow-lg">
                        <form id="wizardForm" @submit.prevent="finalSubmit" class="space-y-8">
                            ...
                            <?php if ($class_id_from_url): ?>
                                <input type="hidden" name="class_id" value="<?= htmlspecialchars($class_id_from_url) ?>">
                            <?php endif; ?>
                            <!-- Breadcrumb Stepper -->
                            ...
                            <div class="relative min-h-[550px]">
                                <div x-show="step === 1" x-transition.opacity class="space-y-6 animate-reveal">
                                ...
                                <div x-show="step === 2" x-transition.opacity style="display: none;" class="space-y-6 animate-reveal">
                                ...
                                <div x-show="step === 3" x-transition.opacity style="display: none;" class="space-y-6 animate-reveal">
                                    <div class="pb-4 border-b border-gray-200/80">
                                    ...
                                    <div class="grid md:grid-cols-2 gap-8">
                                        <div class="bg-slate-50 p-6 rounded-2xl">
                                            ...
                                            <div class="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                                                <?php foreach ($myClasses as $class): ?>
                                                    <label class="flex items-center gap-4 p-4 bg-white border border-slate-100 rounded-2xl cursor-pointer hover:border-orange-400 transition-all">
                                                        <input type="checkbox" name="target_classes[]" value="<?= $class['ClassID'] ?>" class="w-5 h-5 rounded text-orange-600 focus:ring-0" @change="fetchGroupPreview()">
                                                        <div>
                                        ...
                                        <div class="space-y-8">
                                            ...
                                            <!-- Group Preview -->
                                            <div x-show="mode === 'Group' && grouping === 'Auto'" x-transition class="bg-white p-6 rounded-2xl border border-slate-200 mt-8">
                                                ...
                                                <div class="bg-slate-50 p-4 rounded-xl border border-slate-200 max-h-60 overflow-y-auto space-y-4 custom-scrollbar pr-2">
                                                    <div x-show="isPreviewLoading" class="text-center text-slate-400 p-4 animate-pulse">Loading Preview...</div>
                                ...
                                <div x-show="step === 4" x-transition.opacity style="display: none;" class="space-y-6 animate-reveal">
                                    <div class="pb-4 border-b border-gray-200/80">
                                    ...
                                    <div class="flex h-[60vh] border border-slate-200 rounded-2xl overflow-hidden shadow-inner bg-slate-50">
                                        ...
                                        <div class="w-1/3 bg-slate-100 border-r border-slate-200 p-4 flex flex-col">
                                            <div class="flex justify-between items-center mb-4">
                                                ...
                                                <button x-show="roster.length > 0" @click="randomlyAssignRemainder()" type="button" title="Randomly assign remaining students to groups with leaders" class="text-orange-500 hover:text-orange-700 p-1 rounded-full hover:bg-orange-100 transition-colors">
                                            ...
                                            <div class="flex-1 overflow-y-auto space-y-2 pr-2 custom-scrollbar">
                                                <template x-for="stu in roster" :key="stu.MasterID">
                                                    <div class="bg-white p-3 rounded-lg border border-slate-200 hover:border-orange-500 cursor-pointer group transition-all"
                                                        :class="selectedStudent === stu ? 'border-blue-500 bg-blue-50' : ''"
                                                ...
                                                <div x-show="roster.length === 0" class="text-slate-400 text-xs font-bold text-center mt-10">All students assigned!</div>
                                            </div>
                                        </div>
                                                ...
                                                <template x-for="(group, gIndex) in manualGroups" :key="gIndex">
                                                    <div class="bg-slate-50 rounded-2xl p-4 border shadow-sm flex flex-col h-full min-h-[200px] transition-colors" :class="group.members.length > group_limit ? 'border-red-300' : 'border-slate-200'">
                                                        <div class="flex justify-between items-center mb-4 border-b border-slate-200 pb-2">
                                                            ...
                                                            <span class="text-[10px] font-black mr-2" :class="group.members.length > group_limit ? 'text-red-500 animate-pulse' : (group.members.length == group_limit ? 'text-green-500' : 'text-slate-500')">(<span x-text="group.members.length"></span>/<span x-text="group_limit"></span>)</span>
                                                        ...
                                                        <div class="flex-1 space-y-2">
                                                            ...
                                                            <div x-show="selectedStudent" @click="placeStudent(gIndex)" class="border border-dashed border-orange-500/50 bg-orange-500/10 p-3 rounded text-center text-[10px] font-bold text-orange-400 cursor-pointer hover:bg-orange-500/20 animate-pulse transition-all">Click to add <span x-text="selectedStudent?.Full_Name"></span></div>
                                                        </div>
                                                    </div>
                                                </template>
                                            </div>
                                ...
                                <div class="absolute bottom-0 left-0 w-full pt-8 flex justify-between">
                                    <button type="button" x-show="step > 1" @click="prevStep()" class="bg-gray-200 text-gray-700 py-3 px-8 rounded-xl font-bold uppercase text-sm hover:bg-gray-300 transition-all">&larr; Previous</button>
                                    <div class="ml-auto">
                                        ...
                                        <button type="button" x-show="step === 3 && mode === 'Group' && grouping === 'Manual'" @click="goToGroupingStep()" class="bg-orange-500 text-white py-3 px-8 rounded-xl font-bold uppercase text-sm hover:bg-orange-600 transition-all shadow-lg shadow-orange-500/20">Configure Groups &rarr;</button>
                                        <button type="submit" x-show="step === 3 && !(mode === 'Group' && grouping === 'Manual')" class="bg-green-500 text-white py-3 px-8 rounded-xl font-bold uppercase text-sm hover:bg-green-600 transition-all shadow-lg shadow-green-500/20">
                                            <span x-text="editMode ? 'Update Activity' : 'Publish Activity'"></span>
                                        </button>
                                        <button type="submit" x-show="step === 4" class="bg-green-500 text-white py-3 px-8 rounded-xl font-bold uppercase text-sm hover:bg-green-600 transition-all shadow-lg shadow-green-500/20">
                            ...
                            <div class="min-h-[450px]">
                                ...
                                <div x-show="previewTab === 'details'" x-transition.opacity>
                                    <div class="bg-slate-50/70 rounded-xl p-4">
                                        <div class="h-40 bg-slate-200 rounded-lg flex items-center justify-center mb-4 relative overflow-hidden"><iframe x-show="pdfUrl" :src="pdfUrl" class="absolute inset-0 w-full h-full bg-white"></iframe><p x-show="!pdfUrl" class="text-xs text-slate-400 font-bold uppercase">PDF Preview</p></div>
                                ...
                                <!-- State 2: Logistics -->
                                <div x-show="previewTab === 'logistics'" x-transition.opacity style="display: none;">
                                    ...
                                    <div class="space-y-3 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
                                        <template x-for="(item, i) in selectedItems" :key="item.id">
                                            <div class="bg-white p-3 rounded-xl shadow-sm border border-slate-100">
                                                ...
                                                <div x-show="item.is_scalable == 1" class="mt-2 pt-2 border-t border-slate-100">
                                        ...
                                        <div x-show="selectedItems.length === 0" class="text-center text-slate-400 text-xs italic pt-20">No items added yet. Click items from the inventory to add them here.</div>
                                        </div>
                                    </div>

                                <!-- State 3: Settings -->
                                <div x-show="previewTab === 'settings'" x-transition.opacity style="display: none;">
                                    <div class="text-center text-slate-400 pt-20">
                                        <p class="font-bold">Settings Preview</p>
                                        <p class="text-xs italic">This will be implemented later.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('activityWizard', (inventoryData, categoryData, editData = null) => ({
                step: 1,
                previewTab: 'details',
                showFilters: false,
                assetType: 'all',
                ...
                // New properties for live preview
                title: '',
                description: '',
                deadline: '',
                ...
                init() {
                    if (editData) {
                        ...
                        this.selectedItems = editData.requirements.map(req => {
                            ...
                            return {
                                ...
                                is_scalable: fullItem ? fullItem.is_scalable : 0,
                                is_consumable: fullItem ? fullItem.is_consumable : 0,
                                unit: fullItem ? fullItem.Unit : null,
                                variants: fullItem ? fullItem.variants : [],
                                selectedVariantId: req.VariantID || null 
                            };
                        });

                        // Check the assigned class checkboxes
                        this.$nextTick(() => {
                            editData.assignments.forEach(classId => {
                                document.querySelector(`input[name="target_classes[]"][value="${classId}"]`)?.setAttribute('checked', true);
                            });
                        });
                    }
                    ...
                    this.$watch('assetType', () => {
                        // When asset type changes, reset the category filter to prevent invalid states
                ...
                get filteredInventory() {
                    ...
                    return this.fullInventory.filter(item => {
                        ...
                        // The getInventoryShop function in operation.php adds an Asset_Type key.
                ...
                addItem(item) {
                    if (!this.selectedItems.some(i => i.id === item.ItemID)) {
                    ...
                    this.previewTab = 'logistics';
                },
                removeItem(i) { this.selectedItems.splice(i, 1); },

            nextStep() {
                if (this.step === 1) {
                    if (!this.title.trim() || !this.deadline.trim()) {
                        showToast('Please provide a title and deadline before proceeding.', 'error');
                        return;
                    }
                }
                ...
                async fetchGroupPreview() {
                    if (this.mode !== 'Group' || this.grouping !== 'Auto') {
                        this.groupPreview = [];
                        return;
                    }
                    ...
                    if (classIDs.length === 0 || this.group_limit < 1) {
                        this.groupPreview = [];
                        return;
                    }
                    ...
                    try {
                        ...
                        if (result.status === 'success') {
                            this.groupPreview = result.data;
                        } else { throw new Error(result.error || 'Failed to fetch preview.'); }
                    } catch (e) {
                ...
                finalSubmit() {
                    if (this.mode === 'Group' && this.grouping === 'Manual') {
                        this.validateAndSubmitWithGroups();
                    } else {
                        this.submitForm();
                    }
                },
    ...
    if (classIDs.length === 0) {
        showToast('Please assign this activity to at least one class.', 'error');
        return;
    }
    ...
    if (group.members.length >= limit) {
        showToast(`This group is full! (Limit: ${limit})`, 'error');
        return;
    }
    ...
    this.selectedStudent = null;
                ...
                randomlyAssignRemainder() {
                    ...
                    // We'll distribute students into groups that already have members (our leaders)
                    const targetGroups = this.manualGroups.filter(g => g.members.length > 0);

                    if (targetGroups.length === 0) {
                        showToast('Please create groups and assign a leader to each before auto-assigning.', 'error');
                        return;
                    }

                    // Shuffle the remaining students for random distribution
                    ...
                    while (studentsToAssign.length > 0) {
                        const student = studentsToAssign.shift(); // Get next random student

                        let assigned = false;
                        // This loop will try to find a group that is not full, starting from the current groupIndex
                        for (let i = 0; i < targetGroups.length; i++) {
                            let currentGroup = targetGroups[groupIndex];
                            
                            if (currentGroup.members.length < this.group_limit) {
                                currentGroup.members.push(student);
                                this.roster = this.roster.filter(s => s.MasterID !== student.MasterID);
                                assigned = true;
                                groupIndex = (groupIndex + 1) % targetGroups.length;
                                break; // Student assigned, break from the inner for-loop
                            }
                            groupIndex = (groupIndex + 1) % targetGroups.length;
                        }

                        if (!assigned) {
                            showToast('All groups are full. Some students remain unassigned.', 'error');
                            studentsToAssign.unshift(student);
                            break; // Exit the while-loop
                        }
                    }
                    this.selectedStudent = null; // Deselect any student that was highlighted
                    showToast('Remaining students have been randomly assigned.', 'success');
                },

validateAndSubmitWithGroups() {
    // 1. CHECK: Are there students left in the roster?
    if (this.roster.length > 0) {
        showToast(`You have ${this.roster.length} unassigned student(s) left to assign.`, 'error');
        return;
    }
    ...
    if (leaderlessGroups.length > 0) {
        ...
        showToast(`Missing leaders for the following groups: ${names}`, 'error');
        return;
    }
    ...
    this.submitForm();
                ...
                submitForm() {
                    ...
                    // Append the auto-generated group preview data if it exists
                    if (this.mode === 'Group' && this.grouping === 'Auto' && this.groupPreview.length > 0) {
                        formData.append('auto_groups_json', JSON.stringify(this.groupPreview));
                    }

                    // Append requirements data to FormData
                    this.selectedItems.forEach((item, index) => {
                        formData.append(`requirements[${index}][id]`, item.id);
                        formData.append(`requirements[${index}][qty]`, item.qty);
                        if (item.selectedVariantId) {
                            formData.append(`requirements[${index}][selectedVariantId]`, item.selectedVariantId);
                        }
                    });
